...
#define PWM_HI 70
#define PWM_LOW 35
#define PWM_HIGH_INP33 70
#define PWM_LOW_INP33 35

static uint32_t dataBuffer[32 * (NEOPIXEL_NB + 1)];
...
void neopixelSetup(void) {
  ...
  if (HAL_TIM_PWM_Init(&htimneopixel) != HAL_OK)
    Error_Handler();
  if (HAL_TIM_PWM_ConfigChannel(&htimneopixel, &sConfigOC, TIM_CHANNEL_1) !=
      HAL_OK)
    Error_Handler();
  ...
  for (uint16_t i = 0; i++ < NEOPIXEL_NB;) {
    ledFormats[i] = FORMAT_GRB;
  }
...
static void _sendNeopixel(void) {
  while (HAL_DMA_GetState(&hdma_timneopixel_ch1) == HAL_BUSY)
    ;
  for (int i = (NEOPIXEL_NB + 1) * 24;
       i < (NEOPIXEL_NB + 1) * 24 + NEOPIXEL_RESET_TIMER_TICK; i++) {
    data[i] = 0;
  }
  uint32_t timeout = HAL_GetTick();
  while (!datasentflag) {
  ...
  HAL_StatusTypeDef status =
      HAL_TIM_PWM_Start_DMA(&htimneopixel, TIM_CHANNEL_1, data,
                            totalBits + NEOPIXEL_RESET_TIMER_TICK);
...
uint32_t getColorValue(colorRGB_t color, ledFormat_t fmt) {
  ...
  case FORMAT_RGBW:
    // On peut faire une conversion simple : prendre le minimum des trois canaux
    // comme blanc, soustraire aux autres
    {
...
static void writeBitsToBuffer(size_t startBitIndex, uint32_t colorValue,
                              uint8_t numBits,ledFormat_t fmt) {

  for (int i = numBits - 1; i >= 0; i--) {
    size_t index = startBitIndex + (numBits - 1 - i);
    if (fmt == FORMAT_GRB_INP33 ){
      data[index] = (colorValue & (1 << i)) ? PWM_HIGH_INP33 : PWM_LOW_INP33;
    }
    else { 
...
void neopixelSetAllLeds(colorRGB_t color) {
  neopixelWaitForIdle();
  totalBits = 0;
  for (uint16_t i = 0; i < NEOPIXEL_NB; i++) {
    ledFormat_t fmt = ledFormats[i];
    uint32_t col = getColorValue(color, fmt);
    uint8_t bits = getBitsPerLed(fmt);
    writeBitsToBuffer(totalBits, col, bits,fmt);
    totalBits += bits;
  }
  _sendNeopixel();
}
void neopixelSetAllLeds(colorRGBA_t color) {
  ...
  for (uint16_t i = 0; i < NEOPIXEL_NB; i++) {
    ledFormat_t fmt = ledFormats[i];
    uint32_t col = getColorValue(RGBA_to_RGB(color), fmt);
    uint8_t bits = getBitsPerLed(fmt);
    writeBitsToBuffer(totalBits, col, bits,fmt);
    totalBits += bits;
  }
...
void neopixelSetAllLeds(colorHSV_t color) {
  ...
  for (uint16_t i = 0; i < NEOPIXEL_NB; i++) {
    ledFormat_t fmt = ledFormats[i];
    uint32_t col = getColorValue(HSV_to_RGB(color), fmt);
    uint8_t bits = getBitsPerLed(fmt);
    writeBitsToBuffer(totalBits, col, bits,fmt);
    totalBits += bits;
  }
...
colorRGB_t HSV_to_RGB(colorHSV_t hsv) {
  ...
  colorRGB_t rgb = {.red = (uint8_t)((r1 + m) * 255),
...
colorRGB_t RGBA_to_RGB(colorRGBA_t rgba) {
  float brightness =
      rgba.alpha / 255.0; // Normalisation de la luminosité (0.0 - 1.0)

  colorRGB_t rgb = {.red = (uint8_t)(rgba.red * brightness),
